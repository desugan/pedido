...
const Produtos: React.FC = () => {
  ...
  const { produtos, total, currentPage, resumo, load, create: createProduto, update: updateProduto } = useProdutos();
  const toast = usePageToast();

  const [form, setForm] = useState({ nome: '', marca: '', valor: 0, saldo: 0 });
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      if (editId) {
        await updateProduto(editId, { nome: form.nome, marca: form.marca });
      ...
      await load();
    } catch { toast.showError('Erro ao salvar produto'); }
  };
    ...
    <>
      <div className="container mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-4">
          ...
          <button type="button" className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl font-semibold"
            onClick={() => { setEditId(null); setForm({ nome: '', marca: '', valor: 0, saldo: 0 }); setShowCreateModal(true); }}>Novo Produto</button>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white rounded-2xl shadow p-4 border border-slate-100">
            ...
            <div className="text-xl font-bold mt-1">{resumo.total_produtos}</div>
          </div>
          <div className="bg-white rounded-2xl shadow p-4 border border-slate-100">
            <div className="text-xs text-slate-500 uppercase tracking-wide">Itens em estoque</div>
            <div className="text-xl font-bold mt-1">{Math.floor(resumo.total_estoque)}</div>
          </div>
          <div className="bg-white rounded-2xl shadow p-4 border border-slate-100">
            <div className="text-xs text-slate-500 uppercase tracking-wide">Valor em estoque</div>
            <div className="text-xl font-bold mt-1">{fmtBRL(resumo.valor_estoque)}</div>
          </div>
          <div className="bg-white rounded-2xl shadow p-4 border border-slate-100">
            <div className="text-xs text-slate-500 uppercase tracking-wide">Valor médio</div>
            <div className="text-xl font-bold mt-1">{fmtBRL(resumo.total_produtos ? resumo.valor_estoque / resumo.total_produtos : 0)}</div>
          </div>
        </div>

        <div className="text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-xl px-4 py-2 mb-4">
          Valor e saldo são gerenciados via Lançamentos. Aqui só é possível alterar nome e marca.
        </div>
            ...
            <tbody>
              {produtos.map((p: any) => (
                <tr key={p.id_produto} className="border-t">
                  ...
                  <td className="px-4 py-2">{fmtBRL(p.valor)}</td>
            ...
            <form onSubmit={submit} className="space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  <input className="border rounded-xl p-2 w-full" value={form.nome} onChange={(e) => setForm({ ...form, nome: e.target.value })} maxLength={255} required />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Marca</label>
                  <input className="border rounded-xl p-2 w-full" value={form.marca} onChange={(e) => setForm({ ...form, marca: e.target.value })} maxLength={255} required />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Valor (R$)</label>
                  <input className="border rounded-xl p-2 w-full" type="number" step="0.01" min={0} value={form.valor || ''} onChange={(e) => setForm({ ...form, valor: Number(e.target.value) })} required />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Saldo inicial</label>
                  <input className="border rounded-xl p-2 w-full" type="number" step="1" min={0} value={form.saldo || ''} onChange={(e) => setForm({ ...form, saldo: Number(e.target.value) })} />
                </div>
              </div>
