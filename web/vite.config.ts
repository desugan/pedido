...
export default defineConfig(({ mode }) => {
  ...
  const apiPort = env.VITE_API_PORT || env.PORT || parsedApiUrl?.port || '5001';
  const frontendPort = Number(env.VITE_PORT || 5171);
  const apiTarget = `${apiProtocol}://${apiHost}:${apiPort}`;

  return {
    plugins: [react()],
    cacheDir: path.join(tmpdir(), 'pedido-web-vite-cache'),
    server: {
      allowedHosts: ['buteco.leaodiesel.com.br'],
      host: true,
      port: frontendPort,
      proxy: {
        '/api': {
