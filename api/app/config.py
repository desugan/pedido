...
class Config:
    ...
    JWT_AUDIENCE = os.getenv("JWT_AUDIENCE", "pedido-web")
    ...
    def get_jwt_private_key() -> str:
        ...
        logger.info("config.py - get_jwt_private_key: looking at %s, exists=%s", key_path, os.path.exists(key_path))
        if os.path.exists(key_path):
            content = open(key_path, "r").read()
            logger.info("config.py - get_jwt_private_key: loaded, length=%s", len(content))
            return content
        logger.error("config.py - get_jwt_private_key: NOT FOUND at %s", key_path)
        return ""
    ...
    def get_jwt_public_key() -> str:
        ...
        logger.info("config.py - get_jwt_public_key: looking at %s, exists=%s", key_path, os.path.exists(key_path))
        if os.path.exists(key_path):
            content = open(key_path, "r").read()
            logger.info("config.py - get_jwt_public_key: loaded, length=%s", len(content))
            return content
        logger.error("config.py - get_jwt_public_key: NOT FOUND at %s", key_path)
        return ""
